...
#                 print"[getAllOfCardsNum] checking card", card
    ...
    def addCard(self, card):  # check that card to be added doesn't already  exist in hand
        if not  self.pileArr[card[0]-1][card[1]-1]:
            self.pileArr[card[0]-1][card[1]-1] = 1
            self.pileSize += 1
    ...
    def removeCard(self, card):
        if self.pileArr[card[0]-1][card[1]-1]: # check that card to be removed exists
            self.pileArr[card[0]-1][card[1]-1] = 0
            self.pileSize -= 1
            self.cardFreqs[card[1]-1] -= 1
            return True
        return False
    ...
    def removeCards(self , cardList,numToDecrease=None):
        for card in cardList:
